...
Class Ikhti extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('ikhti_model');
        $am = $this->ikhti_model->akses();
    ...
    public function detail($id)
    {
        ...
        $config['base_url'] = 'http://localhost:8080/KPU/ikhti/detail/'. $id .'/index';
        $config['total_rows'] = $this->ikhti_model->countAllRecord();
        $config['per_page'] = 2;
        ...
        $data['komen'] = $this->ikhti_model->komenpage($config['per_page'] , $data['start']);

        $data['title'] = 'Info Detail Kandidat';
        $data['kandidat'] = 'HiMTI/';
        $data['calon'] = 'HiMTI';
        $data['reply'] = 'reply_himti';
        $data['cekkomen'] = 'komentar_himti';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
    ...
    public function hapuskomentar()
    {
        ...
        $this->ikhti_model->hapuskomentar();  
    }
