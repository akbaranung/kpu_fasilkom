...
class Khsi_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from data_himsisfo");
        ...
        foreach($user->result() as $rowa) {
            ...
            <a href='khsi/detail/$rowa->id' class='tooltip-error' data-rel='tooltip' title='Detail' ><button class='btn btn-info ace-icon fa fa-user-circle'> Detail</button></a> 
            </center>";
            
            $row = array();
            $row[] = $i;
            $row[] = '<img width="200" height="150" src="'. base_url() .'/kandidat/HIMSISFO/'. $rowa->img .'" >';
            $row[] = $rowa->nama;
            $row[] = $rowa->no_urut;
            $row[] = $rowa->slogan;
            $row[] = $rowa->visi;
    ...
    public function add()
    {
        ...
        $sql = "SELECT * FROM data_himsisfo WHERE id ='$id'";
        $result =$this->db->query($sql);

        if ($result->num_rows() > 0){
            if($this->db->simple_query("UPDATE data_himsisfo
            ...
            } else if($this->db->simple_query("INSERT INTO data_himsisfo(nama, no_urut, slogan, visi, misi)
        ...
        $action = 'Add / Update Data Himsisfo';
        addLog($action);
    ...
    public function hapus()
    {
        $id = $this->input->post('id');
            ...
            if($this->db->simple_query("DELETE FROM data_himsisfo WHERE id = '$id'")){       
                $data['msg'][0] = "hapus";
    ...
    {
        $akses = $this->db->get_where('menu_data_kandidat', ['url' => 'khsi' ])->row_array();
        $am = $akses['id'];
