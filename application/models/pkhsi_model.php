...
class Pkhsi_model extends CI_Model {
    ...
    public function add()
    {
        $this->form_validation->set_rules('txtNimPemilih', 'NIM', 'required|trim|is_unique[hp_khsi.nim_pemilih]');
        ...
        $nourut = $this->input->post('txtNoUrutKandidat');
            ...
            if ($this->form_validation->run() == false) {
                $data['msg'][0] = "repeat";
                $data['msg'][1] = "Anda Sudah Memilih Kandidat Himsisfo.....";
            } else if($this->db->simple_query("INSERT INTO hp_khsi(kode_pemilih, nama_pemilih, nim_pemilih, email_pemilih, id_kandidat, nama_kandidat, no_urut, status)
                    VALUES ('$kode' ,'$nama', '$nim', '$email', '$id', '$kandidat', '$nourut', 'Belum Sah' )")){       
                $data['msg'][0] = "ok";
                $data['msg'][1] = "Pemilihan Kandidat Himsisfo Telah DiTetapkan.....";
            } else {
                $error = $this->db->error();
