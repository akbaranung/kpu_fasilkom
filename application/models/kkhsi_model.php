...
class Kkhsi_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from komentar_himsisfo");
    ...
    public function add()
    {
        ...
        $sql = "SELECT * FROM komentar_himsisfo WHERE id ='$id'";
        $result =$this->db->query($sql);

        if ($result->num_rows() > 0){
            if($this->db->simple_query("UPDATE komentar_himsisfo
    ...
    {
        $akses = $this->db->get_where('menu_data_komentar', ['url' => 'kkhsi' ])->row_array();
        $am = $akses['id'];
