...
class Rhpkhsi_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from hp_khsi WHERE status = 'Sah'");
    ...
    public function add()
    {
        ...
        $sql = "SELECT * FROM hp_khsi WHERE id ='$id'";
        $result =$this->db->query($sql);

        if ($result->num_rows() > 0){
            if($this->db->simple_query("UPDATE hp_khsi
    ...
    public function hapus()
    {
        $id = $this->input->post('id');
            ...
            if($this->db->simple_query("DELETE FROM hp_khsi WHERE id = '$id'")){       
                $data['msg'][0] = "hapus";
        ...
        $action = 'Delete Report Pemungutan Suara Himsisfo';
        addLog($action);
    ...
    {
        $akses = $this->db->get_where('menu_report', ['url' => 'rhpkhsi' ])->row_array();
        $am = $akses['id'];
