...
class Control4_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from user WHERE is_active = 4 AND role_id != 1 AND role_id != 99");
        ...
        foreach($user->result() as $rowa) {
            ...
            <a href='control4/detail/$rowa->id' class='tooltip-error' data-rel='tooltip' title='Detail' ><button class='btn btn-info ace-icon fa fa-user-circle'> Detail</button></a> 
            </center>";
    ...
    public function add()
    {
        ...
        $action = 'Update Data Peserta Non Fasilkom';
        addLog($action);
    ...
    public function hapus()
    {
        ...
        $action = 'Delete Data Peserta Non Fasilkom';
        addLog($action);
    ...
    {
        $akses = $this->db->get_where('menu_akses_peserta', ['url' => 'control4' ])->row_array();
        $am = $akses['id'];
