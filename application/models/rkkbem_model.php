...
class Rkkbem_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from reply_bem");
    ...
    public function add()
    {
        ...
        $sql = "SELECT * FROM reply_bem WHERE id ='$id'";
        $result =$this->db->query($sql);

        if ($result->num_rows() > 0){
            if($this->db->simple_query("UPDATE reply_bem
