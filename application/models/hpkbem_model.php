...
class Hpkbem_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from hp_kbem WHERE status = 'Belum Sah'");
    ...
    public function add()
    {
        ...
        $sql = "SELECT * FROM hp_kbem WHERE id ='$id'";
        $result =$this->db->query($sql);

        if ($result->num_rows() > 0){
            if($this->db->simple_query("UPDATE hp_kbem
                    SET status = 'Sah'
                    WHERE id='$id'")) {       
                $data['msg'][0] = "ok";
    ...
    public function hapus()
    {
        $id = $this->input->post('id');
            ...
            if($this->db->simple_query("DELETE FROM hp_kbem WHERE id = '$id'")){       
                $data['msg'][0] = "hapus";
        ...
        $action = 'Delete Hasil Pemungutan Suara BEM';
        addLog($action);
    ...
    {
        $akses = $this->db->get_where('menu_hasil', ['url' => 'hpkbem' ])->row_array();
        $am = $akses['id'];
