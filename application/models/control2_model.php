...
class Control2_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from user WHERE is_active = 3 AND role_id != 1 AND role_id != 99");
        
        foreach($user->result() as $rowa) {
            $akses="
            <center>
            <button class='btn btn-success ace-icon fa fa-edit' title='Edit'> Edit</button>
            <button class='btn btn-danger ace-icon fa fa-trash-alt' title='Hapus'> Hapus</button> 
            <a href='control2/detail/$rowa->id' class='tooltip-error' data-rel='tooltip' title='Detail' ><button class='btn btn-info ace-icon fa fa-user-circle'> Detail</button></a> 
            <button class='btn btn-warning ace-icon fa fa-id-card' title='KTM'> KTM</button> 
            <button class='btn btn-primary ace-icon fa fa-envelope' title='Pesan'> Pesan</button> 
            </center>";
    ...
    public function add()
    {
        ...
        $action = 'Update Data Peserta Aktivasi Awal';
        addLog($action);
    ...
    public function hapus()
    {
        ...
        $action = 'Delete Data Peserta Aktivasi Awal';
        addLog($action);
