...
class Kdpm_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from data_dpm");
        ...
        foreach($user->result() as $rowa) {
            ...
            <a href='kdpm/detail/$rowa->id' class='tooltip-error' data-rel='tooltip' title='Detail' ><button class='btn btn-info ace-icon fa fa-user-circle'> Detail</button></a> 
            </center>";
            
            $row = array();
            $row[] = $i;
            $row[] = '<img width="200" height="150" src="'. base_url() .'/kandidat/DPM/'. $rowa->img .'" >';
            $row[] = $rowa->nama;
            $row[] = $rowa->no_urut;
            $row[] = $rowa->slogan;
            $row[] = $rowa->visi;
    ...
    public function add()
    {
        ...
        $sql = "SELECT * FROM data_dpm WHERE id ='$id'";
        $result =$this->db->query($sql);

        if ($result->num_rows() > 0){
            if($this->db->simple_query("UPDATE data_dpm
            ...
            } else if($this->db->simple_query("INSERT INTO data_dpm(nama, no_urut, slogan, visi, misi)
    ...
    public function hapus()
    {
        $id = $this->input->post('id');
            ...
            if($this->db->simple_query("DELETE FROM data_dpm WHERE id = '$id'")){       
                $data['msg'][0] = "hapus";
