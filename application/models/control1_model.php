...
class Control1_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from user WHERE is_active = 0 AND role_id != 1 AND role_id != 99");
        ...
        foreach($user->result() as $rowa) {
            ...
            <a href='control1/detail/$rowa->id' class='tooltip-error' data-rel='tooltip' title='Detail' ><button class='btn btn-info ace-icon fa fa-user-circle'> Detail</button></a> 
            </center>";
    ...
    public function add()
    {
        ...
        $action = 'Update Data Peserta Baru';
        addLog($action);
    ...
    public function hapus()
    {
        ...
        $action = 'Delete Data Peserta Baru';
        addLog($action);
    ...
    {
        $akses = $this->db->get_where('menu_akses_peserta', ['url' => 'control1' ])->row_array();
        $am = $akses['id'];
