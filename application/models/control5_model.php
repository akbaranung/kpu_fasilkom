...
class Control5_model extends CI_Model {
    ...
    {
        $i = 1;
        $user = $this->db->query("SELECT * from user WHERE is_active = 2 AND role_id != 1 AND role_id != 99");
        ...
        foreach($user->result() as $rowa) {
            ...
            <a href='control5/detail/$rowa->id' class='tooltip-error' data-rel='tooltip' title='Detail' ><button class='btn btn-info ace-icon fa fa-user-circle'> Detail</button></a> 
            </center>";
    ...
    public function add()
    {
        ...
        $action = 'Update Data Peserta Nonaktif';
        addLog($action);
    ...
    {
        $akses = $this->db->get_where('menu_akses_peserta', ['url' => 'control5' ])->row_array();
        $am = $akses['id'];
